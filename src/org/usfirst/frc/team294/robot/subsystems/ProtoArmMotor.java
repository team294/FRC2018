...
public class ProtoArmMotor extends Subsystem {
	...
	private double DEGREES_PER_TICK = RobotMap.degreesToTicks;
	private double TICKS_PER_DEGREE = RobotMap.ticksToDegrees;
	...
	**/
    public int getArmPot() {				
    	int potValue = armMotor.getSelectedSensorPosition(0) + 245; //- robotPrefs.getInt("countAtZeroDegrees", 500); //Removed zero for testing purposes
    ...
    private void setArmPosition(double position) {
    	position -= 245; // Zero for testing purposes, replace with constant
    	armMotor.set(ControlMode.Position, position); // TODO: fix closed loop control
    }
    
    /**
     * Sets the position of the arm according to the smart dashboard
     */
    public void setArmFromSmartDashboard() {
    	double angle = SmartDashboard.getNumber("set Arm Angle", 0);
    	setArmPosition(angle); // For closed-loop testing purposes, passing in encoder values instead of angles
    }
